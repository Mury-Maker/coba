        ...
        @auth
            @if(auth()->user()->role === 'admin')
                ...
                <div class="flex items-center justify-between flex-wrap mb-4 gap-2">
                    ...
                    {{-- Search Form --}}
                    <form id="searchForm"
                        data-url="{{ route('docs', ['category' => $currentCategory, 'page' => $currentPage]) }}"
                        method="GET"
                    ...
                    <tbody id="useCaseListTableBody">
                        @forelse($useCases as $useCase)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="py-2 px-4 border-r border-b">{{ ($useCases->currentPage() - 1) * $useCases->perPage() + $loop->iteration }}</td>
                            ...
                            <div class="mt-2 sm:mt-0">
                                <span class="relative inline-flex shadow-sm rounded-md">
                                    ...
                                    @foreach ($useCases->getUrlRange(1, $useCases->lastPage()) as $page => $url)
                                        <a href="{{ $url . '&per_page=' . $per_page . '&search=' . $search_term }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium {{ $page == $useCases->currentPage() ? 'text-white bg-blue-600 border-blue-600' : 'text-gray-700 bg-white border-gray-300 hover:text-blue-600' }} border">
                                            {{ $page }}
                                        </a>
                                    @endforeach
                                    ...
                                    @if ($useCases->hasMorePages())
                                        <a href="{{ $useCases->nextPageUrl() . '&per_page=' . $per_page . '&search=' . $search_term }}" rel="next" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md hover:text-gray-700 focus:z-10 focus:outline-none focus:border-blue-300 focus:ring-blue-200 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150">
